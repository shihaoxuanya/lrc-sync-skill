...
将 LRC 直接写入 MP3（不会重新编码音频；输出为新文件，避免修改原文件）：

    python -m pip install mutagen
    python lrc-sync/scripts/lrc_tool.py embed --audio "song.mp3" --lrc "work/song.lrc" -o "work/song-with-lyrics.mp3"

该命令写入标准 ID3 `USLT` 和兼容性较好的 `TXXX:LYRICS` 字段。不同播放器对内嵌歌词字段的支持不同；若播放器不显示，请保留同名外置 `.lrc`。

QQ 音乐本地播放建议生成同名文件目录：

    python lrc-sync/scripts/lrc_tool.py bundle --audio "song.mp3" --lrc "song.lrc" -o "work/qqmusic-song"

目录中会生成 `song.mp3` 和同名 `song.lrc`，可直接将 MP3 导入 QQ 音乐；QQ 音乐会自动匹配同目录歌词。
```
