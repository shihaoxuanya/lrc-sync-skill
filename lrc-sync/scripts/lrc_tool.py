...
def embed_lyrics(audio: str | Path, lyrics: str, output: str | Path, *, force: bool = False) -> None:
    """Write LRC text into an MP3's ID3 lyrics fields without re-encoding audio."""
    source = local_file(audio)
    if source.suffix.lower() != ".mp3":
        raise ValueError("目前只能将歌词写入 MP3；其他格式的歌词封装方式不同。")
    target = Path(output)
    if target.exists() and not force:
        raise ValueError(f"输出已存在：{target}。请改名，或明确使用 --force。")
    target.parent.mkdir(parents=True, exist_ok=True)
    try:
        ...
        raise ValueError("缺少 mutagen 依赖，请运行：python -m pip install mutagen") from exc
    try:
        tags = ID3(str(source))
    ...
    for line in lyrics.splitlines():
        match = re.match(r"^(?:\ufeff)?((?:\[\d+:[0-5]\d(?:\.\d{1,3})?\])+)(.*)$", line)
        if not match:
            continue
        text = match[2]
        for stamp in re.findall(r"\[(\d+):([0-5]\d)(?:\.(\d{1,3}))?\]", match[1]):
            fraction = (stamp[2] or "0").ljust(3, "0")
            synchronized.append((text, (int(stamp[0]) * 60 + int(stamp[1])) * 1000 + int(fraction)))
    if synchronized:
        synchronized.sort(key=lambda item: item[1])
        tags.add(SYLT(encoding=3, lang="eng", format=1,
                      type=1, desc="", text=synchronized))
    tags.add(TXXX(encoding=3, desc="LYRICS", text=lyrics))
    tags.save(str(target), v2_version=3)


def bundle_for_player(audio: str | Path, lrc: str | Path, output_dir: str | Path, *, force: bool = False) -> tuple[Path, Path]:
    """Create a same-basename MP3/LRC pair for players such as QQ Music."""
    source_audio = local_file(audio)
    source_lrc = local_file(lrc)
    target_dir = Path(output_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    stem = source_audio.stem
    target_audio = target_dir / f"{stem}{source_audio.suffix.lower()}"
    target_lrc = target_dir / f"{stem}.lrc"
    if not force and (target_audio.exists() or target_lrc.exists()):
        raise ValueError(f"输出已存在：{target_audio} 或 {target_lrc}。请改目录，或明确使用 --force。")
    target_audio.write_bytes(source_audio.read_bytes())
    target_lrc.write_text(source_lrc.read_text(encoding="utf-8-sig"), encoding="utf-8-sig")
    return target_audio, target_lrc
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("embed", help="将 LRC 写入 MP3 内嵌歌词标签，不重新编码音频")
    p.add_argument("--audio", required=True); p.add_argument("--lrc", required=True)
    p = sub.add_parser("bundle", help="生成音频与同名 LRC，供 QQ 音乐本地导入")
    p.add_argument("--audio", required=True); p.add_argument("--lrc", required=True)
    ...
    try:
        ...
        if command == "init":
            ...
            embed_lyrics(args.audio, read_text(args.lrc), args.output, force=args.force)
            print(f"已将歌词写入 {args.output}；音频未重新编码。")
            return 0
        elif command == "bundle":
            audio_path, lrc_path = bundle_for_player(args.audio, args.lrc, args.output, force=args.force)
            print(f"已生成 QQ 音乐导入目录：{audio_path}；{lrc_path}")
            return 0
        elif command == "shift":
